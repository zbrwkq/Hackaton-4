...
export default function YearsButton({setSelectedIdCountry,countries}) {
    const colors = [
    ...
    return (
        <div style={{ margin:'50px auto', width: '95%', position: 'relative' }}>
            <IoIosArrowBack size={40} onClick={handlePrev} style={{ visibility: startIndex === 0 ? 'hidden' : 'visible', position: 'absolute', top: '50%', left: '15%', transform: 'translateY(-50%)',cursor: 'pointer' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 40, justifyContent: 'center'}}>
                {countries.slice(startIndex, startIndex + 5).map((country, index) => (
                    <Button 
                        ...
                        text={country.name}
                        onClick={(e) => setSelectedIdCountry(e.target.value)} // Utilisez une fonction de rappel pour gérer le clic
                        value={country.id}
            ...
            <IoIosArrowForward size={40} onClick={handleNext} style={{ visibility: startIndex + 5 >= countries.length ? 'hidden' : 'visible', position: 'absolute', top: '50%', right: '15%', transform: 'translateY(-50%)', cursor: 'pointer' }} />
        </div>
    );
}
